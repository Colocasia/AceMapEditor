...
采用**DLL + UPM Package结构**，核心逻辑编译为.NET Standard DLL，Unity Package引用DLL，便于未来扩展到其他引擎（如Godot）：
...
│
├── Core/                               # 核心逻辑层（.NET Standard 2.1）
│   ├── AceMapEditor.Core.csproj        # .NET项目文件
│   ├── README.md                       # Core层说明
│   ├── src/                            # 源代码
│   │   ├── Data/                       # 数据模型
│   │   │   ├── MapData.cs              # 地图数据
...
│   │
│   ├── Plugins/                        # 预编译DLL存放目录
│   │   ├── AceMapEditor.Core.dll       # Core DLL（从Core/编译复制）
...
# 未来扩展（示例）
# 添加Godot支持时，在仓库根目录创建 Godot/ 目录（与Packages/平级）：
#
# AceMapEditor/                            # 仓库根目录
# │
# ├── Core/                                # 核心DLL（共享）
# │   └── bin/Release/netstandard2.1/
# │       └── AceMapEditor.Core.dll        ← Unity和Godot都引用这个DLL
# │
# ├── Packages/com.acemapeditor.unity/     # Unity Package
# │
# └── Godot/                               # Godot项目（新建）
#     ├── project.godot                    # Godot项目文件
#     ├── addons/acemapeditor/             # Godot插件
#     │   ├── plugin.cfg                   # Godot插件配置
#     │   ├── Libs/
#     │   │   └── AceMapEditor.Core.dll    ← 从Core/编译复制
#     │   ├── Adapters/                    # Godot适配器实现
#     │   │   ├── GodotSceneViewAdapter.cs # 实现ISceneViewAdapter
#     │   │   ├── GodotGizmosAdapter.cs    # 实现IGizmosAdapter
#     │   │   ├── GodotInputAdapter.cs     # 实现IInputAdapter
#     │   │   └── GodotEditorUIAdapter.cs  # 实现IEditorUIAdapter
#     │   └── Editor/
#     │       ├── MapEditorPlugin.cs       # Godot插件入口
#     │       └── MapEditorDock.cs         # 编辑器Dock窗口
#     └── Scenes/                          # Godot测试场景
#
# 说明：
# - Godot/目录与Packages/平级，是独立的Godot项目
# - 只需实现适配器层（约500-1000行代码）
# - Core逻辑完全复用，无需修改
# - Build/build-core.sh可扩展为同时复制DLL到两个引擎
```

### 目录结构设计说明

#### 1. **Core/** - 核心逻辑层（.NET Standard 2.1 DLL）
- **完全引擎无关**：不依赖Unity、Godot等任何引擎
- **纯C#实现**：标准.NET项目，可在任何.NET环境运行
- **接口驱动**：通过接口定义引擎能力，具体实现由各引擎层提供
- **高性能优化**：支持unsafe代码、System.Numerics.Vector3等高性能特性
- **编译输出**：AceMapEditor.Core.dll（24KB），netstandard2.1目标框架
- **零Unity依赖**：使用System.Numerics而非UnityEngine类型

#### 2. **Packages/com.acemapeditor.unity/** - Unity Package（UPM）
- **Unity特定代码**：所有Unity相关的API调用都在这里
- **适配器实现**：实现Core层定义的接口，桥接Unity API
- **DLL引用**：通过Plugins/目录引用预编译的Core.dll
- **编辑器扩展**：EditorWindow、SceneView、Handles等
- **运行时组件**：MonoBehaviour、ScriptableObject等（按需）
- **URP资源**：Shader、材质、预制体等
- **Assembly Definition**：AceMapEditor.Editor.asmdef引用Core.dll

#### 3. **Build/** - 构建系统
- **build-core.sh**：自动化构建脚本
  - 编译Core DLL（Release配置）
  - 复制到Unity Package Plugins目录
  - 复制到Releases发布目录
- **一键构建**：`bash Build/build-core.sh`
...
- **添加Godot支持**：
  - 创建新的Godot插件项目
  - 引用同一个AceMapEditor.Core.dll
  - 实现Godot版本的Adapter接口
  - Core层代码无需任何修改，直接复用
- **多引擎共存**：不同引擎实现可以共存，共享Core DLL

### Assembly Definition说明

Unity Package使用Assembly Definition (.asmdef)实现模块隔离和编译优化：

#### 1. **Core DLL（预编译）**
- **不使用.asmdef**：Core是独立的.NET Standard 2.1项目
- **编译为DLL**：通过build-core.sh编译生成
- **零Unity依赖**：完全引擎无关
- **unsafe代码**：Core.csproj中配置`<AllowUnsafeBlocks>true</AllowUnsafeBlocks>`

#### 2. **AceMapEditor.Editor.asmdef**（Unity编辑器）
```json
{
  "name": "AceMapEditor.Editor",
  "rootNamespace": "AceMapEditor.Unity.Editor",
  "references": [],
...
- **职责**：Unity编辑器扩展和适配器
- **平台**：仅Editor平台
- **DLL引用**：通过`precompiledReferences`引用Core.dll（位于Plugins/）
- **命名空间**：`AceMapEditor.Unity.Editor`

#### 3. **AceMapEditor.Runtime.asmdef**（Unity运行时，可选）
```json
{
  "name": "AceMapEditor.Runtime",
  "rootNamespace": "AceMapEditor.Unity.Runtime",
  "references": [],
  "includePlatforms": [],
  "excludePlatforms": [],
  "allowUnsafeCode": false,
...
```
- **职责**：运行时组件（如MonoBehaviour，按需）
- **平台**：所有平台
- **DLL引用**：同样引用Core.dll

#### 4. **测试Assembly**
- `AceMapEditor.Tests.Editor.asmdef`：编辑器测试
- `AceMapEditor.Tests.Runtime.asmdef`：运行时测试

#### 优势
- **清晰分离**：Core逻辑完全独立编译
- **快速迭代**：修改Core后只需运行build脚本，Unity自动重新加载DLL
- **跨引擎复用**：同一个DLL可被Unity、Godot等引擎使用
- **版本管理**：DLL可独立版本化和分发
...
- **UnitDefinition**: 单位定义（纯C#类，存储为JSON）
...
#### 数据存储
- 定义文件：`Definitions/Units/*.json`
- 运行时加载：从JSON反序列化为UnitDefinition对象
- Unity层可选：ScriptableObject作为Asset包装（仅用于Inspector编辑）
...
[JsonSerializable(typeof(DoodadDefinition))]
public partial class DefinitionJsonContext : JsonSerializerContext { }

// 序列化工具类
public static class DefinitionSerializer
{
    public static string Serialize(UnitDefinition unit)
    {
        return JsonSerializer.Serialize(
            unit,
            DefinitionJsonContext.Default.UnitDefinition
        );
    }

    public static UnitDefinition LoadUnit(string jsonPath)
    {
...
**Unity层**：
- EditorWindow可视化编辑界面
- 从JSON加载数据 → 编辑 → 保存到JSON
- 可选：ScriptableObject作为Asset容器（仅用于Unity Inspector）
...
#### 存储方式
- **格式**：JSON（System.Text.Json）
- **路径**：`Definitions/Units/*.json`, `Definitions/Abilities/*.json`
- **特性**：
  - 引擎无关（Unity、Godot都能读取）
  - 人类可读（可手动编辑）
  - 版本控制友好（Git diff清晰）
  - AOT友好（源生成器，零反射）
...
```json
// Definitions/Units/Marine.json
{
  "id": "marine",
  "name": "陆战队员",
  "health": 45.0,
  "speed": 2.25,
  "modelPath": "Units/Terran/Marine",
  "abilities": [
    "stimpack",
    "attack"
  ]
